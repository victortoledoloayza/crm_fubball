<?php
/**
 * api/facturacion_marcar.php (POST, JSON body)
 *
...
try {
    PedidoRepository::marcarFacturado($pedidoId, (int) $usuarioSesion['id']);
    echo json_encode(['ok' => true]);
} catch (PedidoTransicionInvalidaException $e) {
    http_response_code(409);
    echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
} catch (Throwable $e) {
    error_log('[facturacion_marcar.php] ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['ok' => false, 'error' => 'Ocurrió un error al marcar el pedido como facturado.']);
}
